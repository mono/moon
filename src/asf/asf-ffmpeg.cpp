...
//	asf_object* obj = NULL;
	...
	while ((index = parser->GetHeaderObjectIndex (&asf_guids_stream_properties, index + 1)) >= 0) {
		...
		if (!file_properties->is_broadcast ()) {
			stream->duration = file_properties->play_duration / 10000 - stream->start_time; /// (10000 /*000 / 1000*/) - file_properties->preroll;
		}
...
int64_t ffmpeg_asf_read_pts(AVFormatContext *s, int ffmpeg_stream_index, int64_t *start_pos, int64_t pos_limit)
{
	...
	packet_index = parser->GetPacketIndex (pos);
...
void AVFormatContext_dump (AVFormatContext* s)
{
	int t = 1;
	...
	FFMPEG_DUMP ("%sav_class = %s\n", tabs, s->av_class == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%siformat = %s\n", tabs, s->iformat == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%soformat = %s\n", tabs, s->oformat == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%spriv_data = %s\n", tabs, s->priv_data == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%spb = <ByteIOContext>\n", tabs);
	FFMPEG_DUMP ("%snb_streams = %u\n", tabs, s->nb_streams);
	FFMPEG_DUMP ("%sstreams = %s\n", tabs, s->streams == NULL ? "NULL" : "!NULL");
	for (guint32 i = 0; i < s->nb_streams; i++)
		AVStream_dump (s->streams [i], 2);
	FFMPEG_DUMP ("%sfilename = %s\n", tabs, s->filename);
	FFMPEG_DUMP ("%stimestamp = %llu\n", tabs, s->timestamp);
	FFMPEG_DUMP ("%stitle = %s\n", tabs, s->title);
	FFMPEG_DUMP ("%sauthor = %s\n", tabs, s->author);
	FFMPEG_DUMP ("%scopyright = %s\n", tabs, s->copyright);
	FFMPEG_DUMP ("%scomment = %s\n", tabs, s->comment);
	FFMPEG_DUMP ("%salbum = %s\n", tabs, s->album);
	FFMPEG_DUMP ("%syear = %i\n", tabs, s->year);
	FFMPEG_DUMP ("%strack = %i\n", tabs, s->track);
	FFMPEG_DUMP ("%sgenre = %s\n", tabs, s->genre);
	FFMPEG_DUMP ("%sctx_flags = %i\n", tabs, s->ctx_flags);
	FFMPEG_DUMP ("%spacket_buffer = %s\n", tabs, s->packet_buffer == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%sstart_time = %llu\n", tabs, s->start_time);
	FFMPEG_DUMP ("%sduration = %llu\n", tabs, s->duration);
	FFMPEG_DUMP ("%sfile_size = %llu\n", tabs, s->file_size);
	FFMPEG_DUMP ("%sbit_rate = %i\n", tabs, s->bit_rate);
	FFMPEG_DUMP ("%scur_st = %s\n", tabs, s->cur_st == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%scur_ptr = %s\n", tabs, s->cur_ptr == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%scur_len = %i\n", tabs, s->cur_len);
	FFMPEG_DUMP ("%scur_pkt = <AVPacket>\n", tabs);
	FFMPEG_DUMP ("%sdata_offset = %llu\n", tabs, s->data_offset);
	FFMPEG_DUMP ("%sindex_built = %i\n", tabs, s->index_built);
	FFMPEG_DUMP ("%smux_rate = %i\n", tabs, s->mux_rate);
	FFMPEG_DUMP ("%spacket_size = %i\n", tabs, s->packet_size);
	FFMPEG_DUMP ("%spreload = %i\n", tabs, s->preload);
	FFMPEG_DUMP ("%smax_delay = %i\n", tabs, s->max_delay);
	FFMPEG_DUMP ("%sloop_output = %i\n", tabs, s->loop_output);
	FFMPEG_DUMP ("%sflags = %i\n", tabs, s->flags);
	FFMPEG_DUMP ("%sloop_input = %i\n", tabs, s->loop_input);
	FFMPEG_DUMP ("%sprobesize = %u\n", tabs, s->probesize);
	FFMPEG_DUMP ("%smax_analyze_duration = %i\n", tabs, s->max_analyze_duration);
	FFMPEG_DUMP ("%skey = %s\n", tabs, s->key == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%skeylen = %i\n", tabs, s->keylen);
...
{
	char* tabs = g_strnfill (t, '\t');
	//FFMPEG_DUMP ("%s" "AVRational: %p\n", tabs1, s);
	//if (!s) return;
	FFMPEG_DUMP ("%s num = %i, den = %i (as float: %f)\n", tabs, s->num, s->den, (float) s->num / (float) s->den);
 
}
char* AVRational_tostring (AVRational s)
{
	return g_strdup_printf ("<AVRational: num = %i, den = %i, num/den = %f>", s.num, s.den, (float) s.num / (float) s.den);
}

char* AVFrac_tostring (AVFrac s)
{
	return g_strdup_printf ("<AVFrac: val = %lld, num = %lld, den = %lld, num/den = %f>", s.val, s.num, s.den, s.val + (float) s.num / (float) s.den);
}
...
void AVStream_dump (AVStream* s, int t)
{
	...
	if (!s) return;
	FFMPEG_DUMP ("%sindex = %i\n", tabs, s->index);
	FFMPEG_DUMP ("%sid = %i\n", tabs, s->id);
	FFMPEG_DUMP ("%scodec = %s\n", tabs, s->codec == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sstream_copy = %i\n", tabs, s->stream_copy);
	FFMPEG_DUMP ("%sdiscard = %i\n", tabs, s->discard);
	FFMPEG_DUMP ("%squality = %f\n", tabs, s->quality);
	FFMPEG_DUMP ("%sstart_time = %llu\n", tabs, s->start_time);
	FFMPEG_DUMP ("%sduration = %llu\n", tabs, s->duration);
	FFMPEG_DUMP ("%slanguage = %s\n", tabs, s->language);
	FFMPEG_DUMP ("%sneed_parsing = %i\n", tabs, s->need_parsing);
	FFMPEG_DUMP ("%sparser = %s\n", tabs, s->parser == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%scur_dts = %llu\n", tabs, s->cur_dts);
	FFMPEG_DUMP ("%slast_IP_duration = %i\n", tabs, s->last_IP_duration);
	FFMPEG_DUMP ("%slast_IP_pts = %llu\n", tabs, s->last_IP_pts);
	FFMPEG_DUMP ("%sindex_entries = %s\n", tabs, s->index_entries == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%snb_index_entries = %i\n", tabs, s->nb_index_entries);
	FFMPEG_DUMP ("%sindex_entries_allocated_size = %u\n", tabs, s->index_entries_allocated_size);
	FFMPEG_DUMP ("%snb_frames = %llu\n", tabs, s->nb_frames);
	FFMPEG_DUMP ("%spts_buffer = %s\n", tabs, s->pts_buffer == NULL ? "NULL" : "!NULL");
...
void AVCodecContext_dump (AVCodecContext* s, int t)
{
	...
	FFMPEG_DUMP ("%sextradata = %s\n", tabs, s->extradata == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sdraw_horiz_band = %s\n", tabs, s->draw_horiz_band == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%srtp_callback = %s\n", tabs, s->rtp_callback == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sget_buffer = %s\n", tabs, s->get_buffer == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%srelease_buffer = %s\n", tabs, s->release_buffer == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%src_override = %s\n", tabs, s->rc_override == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sslice_offset = %s\n", tabs, s->slice_offset == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%ssample_aspect_ratio = %s\n", tabs, AVRational_tostring (s->sample_aspect_ratio));
	FFMPEG_DUMP ("%scoded_frame = %s\n", tabs, s->coded_frame == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%sdebug = %i\n", tabs, s->debug);
	FFMPEG_DUMP ("%sdebug_mv = %i\n", tabs, s->debug_mv);
	FFMPEG_DUMP ("%serror = %s\n", tabs, s->error == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sget_format = %s\n", tabs, s->get_format == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sinternal_buffer = %s\n", tabs, s->internal_buffer == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sintra_matrix = %s\n", tabs, s->intra_matrix == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%sinter_matrix = %s\n", tabs, s->inter_matrix == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%sstream_codec_tag = %u\n", tabs, s->stream_codec_tag);
	FFMPEG_DUMP ("%sscenechange_threshold = %i\n", tabs, s->scenechange_threshold);
	FFMPEG_DUMP ("%slmin = %i\n", tabs, s->lmin);
	FFMPEG_DUMP ("%slmax = %i\n", tabs, s->lmax);
	FFMPEG_DUMP ("%spalctrl = %s\n", tabs, s->palctrl == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%snoise_reduction = %i\n", tabs, s->noise_reduction);
	FFMPEG_DUMP ("%sreget_buffer = %s\n", tabs, s->reget_buffer == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sexecute = %s\n", tabs, s->execute == NULL ? "NULL" : "!NULL");
	FFMPEG_DUMP ("%sthread_opaque = %s\n", tabs, s->thread_opaque == NULL ? "NULL" : "!NULL");
	...
	FFMPEG_DUMP ("%sskip_loop_filter = %i\n", tabs, s->skip_loop_filter);
	FFMPEG_DUMP ("%sskip_idct = %i\n", tabs, s->skip_idct);
	FFMPEG_DUMP ("%sskip_frame = %i\n", tabs, s->skip_frame);
