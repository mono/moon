...
bool
Keyboard::KeyIsChar (ModifierKeys modifiers, Key key)
{
	if ((modifiers & ~ModifierKeyShift) != 0)
		return false;
	
	if (key >= KeyNUMPAD0 && key <= KeyNUMPAD9 && modifiers == ModifierKeyNone)
		return true;
	
	if ((key >= KeyDIGIT0 && key <= KeyDIGIT9) || (key >= KeyA && key <= KeyZ))
		return true;
...
gunichar
Keyboard::KeyToChar (ModifierKeys modifiers, Key key)
{
	// FIXME: erm... the Key enum seems to be missing a lot of keys
	gunichar c = 0;
	
	if (key >= KeyNUMPAD0 && key <= KeyNUMPAD9) {
		c = '0' + (key - KeyNUMPAD0);
	} else if (key >= KeyDIGIT0 && key <= KeyDIGIT9) {
		c = '0' + (key - KeyDIGIT0);
	} else if (key >= KeyA && key <= KeyZ) {
		if ((modifiers & ModifierKeyShift) || IsKeyPressed (KeyCAPSLOCK))
			c = 'A' + (key - KeyA);
		else
			c = 'a' + (key - KeyA);
	} else {
...
bool
Keyboard::KeyIsMovement (ModifierKeys modifiers, Key key)
{
	switch (key) {
	case KeyPAGEUP:
	case KeyPAGEDOWN:
