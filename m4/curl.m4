...
[
	...
	if test x$with_curl = xembedded; then
		...
		if test x$has_nss = xyes; then
			CUSTOM_SUBDIR_OPTION(curl, [--with-pic --disable-shared --disable-manual --without-libssh2 --disable-ldap --disable-ldaps --without-libidn --without-ssl --with-nss --enable-debug --enable-hidden-symbols])
